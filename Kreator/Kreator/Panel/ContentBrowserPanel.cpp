...
namespace Kreator
{
  ...
  namespace CBP_Utils
  {
    ...
    Ref<Texture> AssetPath(const std::string& fileName)
    {
      return TextureFactory::Create(KreatorLayer::Get().GetClientResorucePath() / "Textures/CBP" / fileName);
    }
  } // namespace CBP_Utils
  ...
  void ContentBrowserPanel::OnImGuiRender(bool &isOpen)
  {
    ImGui::Begin("Content Browser", &isOpen, ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoScrollbar);
    {
      ...
      if (ImGui::BeginTable(UI::GenerateID(), 2 /* Num Columns */, tableFlags, ImVec2(0.0f, 0.0f)))
      {
        ...
        {
          UI::ScopedColor header(ImGuiCol_Header, UI::Color::PropertyField);
  ...
  {
    RETURN_IF (m_copiedAssets.SelectionCount() == 0)
