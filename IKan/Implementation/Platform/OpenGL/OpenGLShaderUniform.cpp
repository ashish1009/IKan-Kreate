...
namespace IKan
{
  ...
  : m_struct(nullptr), m_type(type), m_domain(domain), m_name(name), m_count(count)
  {
    ...
    IK_LOG_DEBUG(LogModule::Shader, "      {0} {1}[{2}] (Size : {3}) ", TypeToString(m_type), m_name, m_count, m_size);
  }
  
  OpenGLShaderUniformDeclaration::OpenGLShaderUniformDeclaration(ShaderDomain domain, ShaderStruct* uniform_struct,
                                                                 const std::string& name, uint32_t count)
  : m_struct(uniform_struct), m_type(OpenGLShaderUniformDeclaration::Type::Struct),m_domain(domain), m_name(name), m_count(count)
  {
  ...
  {
    IK_LOG_DEBUG(LogModule::Shader, "      Destroying : {0} {1}[{2}] (Size : {3}) ", TypeToString(m_type), m_name, m_count, m_size);
  }
  ...
  {
    IK_LOG_DEBUG(LogModule::Shader, "      {0} {1}[{2}]", TypeToString(m_type), m_name, m_count);
  }
  
  OpenGLShaderResourceDeclaration::~OpenGLShaderResourceDeclaration() {
    IK_LOG_DEBUG(LogModule::Shader, "      Destroying : {0} {1}[{2}]", TypeToString(m_type), m_name, m_count);
  }
